...
export function AnimationPreloader() {
  const { initializeAnimation, isAnimationInitialized } = useAnimationContext()
  ...
  // Initialize animation classes and state once
  useEffect(() => {
    if (isInitialized || isAnimationInitialized) return
    ...
    // Initialize the global animation state
    initializeAnimation(animationState)
    setIsInitialized(true)
  }, [initializeAnimation, isAnimationInitialized, isInitialized])

  // This component is invisible - it just preloads the animation
  return (
    <canvas 
      ref={canvasRef}
      style={{ 
