...
import { pagesService, faqCategoriesService } from '@/lib/cms'
import { PageWithSections, FAQCategoryWithFAQs } from '@/types/cms'
...
export default function KnowledgeBasePage() {
  ...
  const fetchPageData = async () => {
    try {
      const [pageDataResult, faqCategoriesResult] = await Promise.all([
        pagesService.getWithSections('knowledge-base'),
        faqCategoriesService.getAll()
      ])
      setPageData(pageDataResult)
      setFaqCategories(faqCategoriesResult)
    } catch (error) {
  ...
  useEffect(() => {
    const handleRefresh = () => {
      console.log('Refreshing knowledge base data...')
      setLoading(true)
      fetchPageData()
    }
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === 'faq-updated') {
        console.log('FAQ updated detected via storage event')
        handleRefresh()
      }
    }
  ...
  // Convert FAQ categories to knowledge categories format
  const knowledgeCategories: KnowledgeCategory[] = useMemo(() => {
    return faqCategories.length > 0 ? faqCategories.map(category => ({
      id: category.id,
      ...
      items: category.faqs.map(faq => ({
        ...
        tags: [] // FAQ items don't have tags in the current structure
      }))
    })) : [
      {
        id: "platform",
  ...
  const faqSection = pageData?.sections.find(s => s.section_type === 'faq')
