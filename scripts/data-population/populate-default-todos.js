...
async function populateTodos() {
  try {
    console.log('Starting to populate default todos...')
    ...
    if (checkError) {
      console.error('Error checking existing todos:', checkError)
      return
    }
    
    if (existingTodos && existingTodos.length > 0) {
      console.log('Todos already exist in database. Skipping population.')
      return
    }
    ...
    console.log(`Successfully inserted ${data.length} default todos`)
  } catch (error) {
