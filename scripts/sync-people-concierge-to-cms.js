...
// Extract exact content from the current /people-concierge page
// This represents the master content that should be in the CMS
const peopleConciergePageMasterContent = {
  page: {
    title: 'People - Concierge Support',
    slug: 'people-concierge',
  ...
  sections: [
    {
      ...
      section_data: {
        ...
        description: 'For organizations that need more than a platform, our Concierge Support team acts as an extension of your own, providing the strategic guidance and technical expertise to design, build, and implement transformative agentic solutions.',
        ctaButtons: [
    ...
    },
    {
      ...
      section_data: {
        ...
        solution: {
          ...
          icon: 'team-line',
          badgeText: 'Our Solution: Your Embedded Team of Experts',
          badgeColor: 'primary',
    ...
    },
    {
      ...
      section_data: {
        ...
        description: 'Our Concierge service is designed for growth-oriented leaders who:',
        icon: 'target-line',
    ...
    },
    {
      ...
      section_data: {
        title: 'Ready to Build Your Agentic Future?',
        description: 'Let\'s discuss how our Concierge Support Team can help you achieve your goals.',
...
async function syncPeopleConciergeToCMS() {
  console.log('🔄 Syncing current /people-concierge page content to CMS...')
  console.log('📋 This will make the current people concierge page content the master source for the CMS')
  
  try {
    // Get or create the people-concierge page
    let { data: page, error: pageError } = await supabase
      .from('pages')
      .select('*')
      .eq('slug', 'people-concierge')
      .single()
    ...
    if (!page) {
      ...
      const { data: newPage, error: createError } = await supabase
        .from('pages')
        .insert(peopleConciergePageMasterContent.page)
        .select()
        .single()
      ...
      const { error: updateError } = await supabase
        .from('pages')
        .update(peopleConciergePageMasterContent.page)
        .eq('id', page.id)
    ...
    // Create new sections with exact content from current people-concierge page
    const sectionsToInsert = peopleConciergePageMasterContent.sections.map(section => ({
      page_id: page.id,
    ...
    console.log('✅ People Concierge page content successfully synced to CMS!')
    console.log(`📊 Created ${insertedSections.length} sections in CMS`)
    console.log('🎯 The current /people-concierge page content is now the master source in the CMS')
    console.log('⚙️ You can now edit this content through: /admin/pages')
    console.log('📝 The /people-concierge page remains exactly as it is - no changes to the frontend')
